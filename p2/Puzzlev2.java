  ...
  public static List<Long> getInvalidIds(Range r) {
    ...
    for (long n = r.start(); n <= r.end(); n++) {
      ...
      for (int i = 0; i < numberString.length() / 2; i++) {
        ...
        String chunk = numberString.substring(0, chunkSize);
        boolean sameChunks = true;
        for (int j = 1; j < numberString.length() / chunkSize; j++) {
          String nextChunk = numberString.substring(j * chunkSize, (j + 1) * chunkSize);
