...
public class Puzzle {
  ...
  public static Position findMin(List<Position> positions, Position in, double minDistance) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (!positionsPending.isEmpty()) {
      ...
      Position target = null;
      for (Position p : positions) {
        Position cp = findMin(positions, p, distanceFloor);
    ...
    for (Set<Position> s : setofSets) {
      setSizes.add(s.size());
    }

    Collections.sort(setSizes);
   
    long answer = setSizes.get(setSizes.size() - 1) * setSizes.get(setSizes.size() - 2) * setSizes.get(setSizes.size() - 3);
